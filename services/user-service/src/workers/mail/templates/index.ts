...
export function buildMail(job: MailJob): { subject: string; text: string; html: string } {
  if (job.type === 'forgot-password') return buildForgotPasswordMail(job);
  if (job.type === 'password-reset-success') return buildPasswordResetSuccessMail(job);
