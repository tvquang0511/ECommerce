...
router.post(
  ...
  rateLimit({
    ...
    windowSeconds: 60,
    identifiers: (req) => {
      const email = typeof (req.body as any)?.email === 'string' ? String((req.body as any).email).toLowerCase() : '';
...
router.post(
  ...
  rateLimit({
    ...
    windowSeconds: 5 * 60,
    identifiers: (req) => {
      const challengeId = typeof (req.body as any)?.challengeId === 'string' ? String((req.body as any).challengeId) : '';
...
router.post('/sessions/:sessionId/revoke', authJwt, asyncHandler(revokeSession));
...
router.post(
  ...
  rateLimit({
    ...
    identifiers: (req) => {
      ...
      const raw = cookieToken ?? (typeof (req.body as any)?.refreshToken === 'string' ? String((req.body as any).refreshToken) : undefined);
      const tokenId = raw && raw.includes('.') ? raw.slice(0, raw.indexOf('.')) : '';
