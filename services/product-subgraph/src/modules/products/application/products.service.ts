...
import { ProductCacheService } from '../infrastructure/product-cache.service';
import { MinioService } from '../infrastructure/minio-product-media.service';
import { CreateProductDto } from '../domain/dto/create-product.dto';
import { UpdateProductDto } from '../domain/dto/update-product.dto';
import { PRODUCT_STATUSES } from '../domain/product.mongo.schema';
import { type ProductCurrency } from '../domain/constants/product-currency';
import { Product, ProductImage } from '../domain/product.domain.type';
import { ProductDocument, ProductModel } from '../domain/product.mongo.schema';
...
export class ProductsService {
  ...
  async create(actor: AuthActor, input: CreateProductDto): Promise<Product> {
    this.ensureActorHasPermission(actor, 'product:create:self');
  ...
  async update(
    ...
    this.ensureActorHasPermission(actor, 'product:update:self');
  ...
  async remove(id: string, actor: AuthActor): Promise<boolean> {
    this.ensureActorHasPermission(actor, 'product:archive:self');
  ...
  async submitForReview(id: string, actor: AuthActor): Promise<Product | undefined> {
    this.ensureActorHasPermission(actor, 'product:publish:self');
  ...
  private ensureActorHasPermission(actor: AuthActor, permission: string): void {
    if (this.isAdmin(actor)) {
      return;
    }
