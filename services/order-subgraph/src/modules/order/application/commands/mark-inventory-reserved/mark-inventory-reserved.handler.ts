  ...
  implements ICommandHandler<MarkInventoryReservedCommand, OrderCommandResult>
{
  ...
  async execute(command: MarkInventoryReservedCommand): Promise<OrderCommandResult> {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      if (
        aggregate.inventoryStatus === OrderInventoryStatusEnum.RESERVED ||
        ...
        return {
          ...
          message: 'Inventory reserved callback already applied.',
        };
      }
    ...
    throw new Error('Inventory reserved callback retry budget exhausted.');
  }
